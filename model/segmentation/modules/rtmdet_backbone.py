...
class RTMDETBackbone(nn.Module):
    def __init__(self, img_size=1024, init_channels=16, device='cpu'):
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(self.init_channels * 8, self.init_channels * 16, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(self.init_channels * 16),
            nn.SiLU(inplace=True),
            RTMDetCSPBlock(self.init_channels * 16, self.init_channels * 16).to(self.device),
        )
    ...
    def forward(self, x):
        ...
        c2 = self.conv2(c1)
        assert c2.shape == (x.shape[0], self.init_channels * 2, x.shape[2] // 4, x.shape[3] // 4), f"c2.shape: {c2.shape}, x.shape: {x.shape}"
        c3 = self.conv3(c2)
        assert c3.shape == (x.shape[0], self.init_channels * 4, x.shape[2] // 8, x.shape[3] // 8), f"c3.shape: {c3.shape}, x.shape: {x.shape}"
        c4 = self.conv4(c3)
        assert c4.shape == (x.shape[0], self.init_channels * 8, x.shape[2] // 16, x.shape[3] // 16), f"c4.shape: {c4.shape}, x.shape: {x.shape}"
        c5 = self.conv5(c4)
        assert c5.shape == (x.shape[0], self.init_channels * 16, x.shape[2] // 32, x.shape[3] // 32), f"c5.shape: {c5.shape}, x.shape: {x.shape}"
